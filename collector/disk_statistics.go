...
func (ds diskStatistics) Update(ch chan<- prometheus.Metric) error {
	...
	for _, disk := range ds.meSession.diskStatistic {
		...
		ch <- prometheus.MustNewConstMetric(ds.numberOfReads.desc, ds.numberOfReads.tipo, float64(disk.NumberOfReads), disk.Location)
		...
		ch <- prometheus.MustNewConstMetric(ds.smartCount.desc, ds.smartCount.tipo, float64(disk.SmartCount1), disk.Location)
		ch <- prometheus.MustNewConstMetric(ds.smartCount.desc, ds.smartCount.tipo, float64(disk.SmartCount2), disk.Location)
